...
void Connection::Start() {
    if (!isAlive()) {
        logger->debug("Start connection socket {}", _socket);
        parser.Reset();
...
void Connection::OnError() {
    if (isAlive()) {
        logger->debug("On error socket {}", _socket);
...
void Connection::OnClose() {
    if (isAlive()) {
        logger->debug("On close socket {}", _socket);
